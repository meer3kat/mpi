/*******************************************************************************
* Run by typing: "mpirun -np p ./quicksort inputfile outputfile pivot_strategy"*
...
void local_merge(int size1, int size2, int* arr1, int* arr2, int* c);
...
int main(int argc, char *argv[]){	
	...
	if (rank == size-1 ) {	istop = n2-1; } /* Make sure the last processor gets all the rest      */
	int local_size;
	...
	//***********************************************
	//now we do local quick sort. and start the clock.
	double t;
	...
	//************************************************
	// now we collect everything on process 0. 
	int k=0;
	int num_get=0;
	int num_tmp;
	int* sorted_array;
...
void local_merge(int size1, int size2, int* arr1, int* arr2, int* c){//allocate c before 
	...
	int j=0;
	int k=0;
	if(arr1[size1-1]<arr2[size2-1]){ //decide if arr1 or arr2 will write the last element in c 
		while(j<size2){
			while(i<size1){
				if(arr1[i]<arr2[j]){
					c[k] = arr1[i];
...
int partition(int* arr, int left, int right, int option){ //partition for local quick sort. 
	int i = left;
	int tmp;
	int pivot;
	if(option == 0){
	...
	tmp = arr[i+1]; //switch the pivot element to the correct place 
	arr[i+1]=arr[right];
	arr[right] = tmp;
...
void quicksort(int* arr, int left, int right, int option){ //local quick sort 
	if (left < right){
